...
div(class="flex items-center gap-2 px-3 py-2.5 cursor-grab active:cursor-grabbing")
  ...
  div(class="flex-1 min-w-0")
    ...
    p(v-if="maxChars" class="text-xs text-gray-400 truncate")
      span(class="font-numeric tabular-nums") {{ maxChars }}
      |
      | chars max
  slot(name="actions")
</template>
...
@Component
export default class FeatureChipContextCompression extends Vue {
  @StringProp(true)
  ...
  public get maxChars(): number | undefined {
    return routerFeatureConfig<FT.FeatureContextCompression>(this.feature).max_chars
  }
}
</script>
