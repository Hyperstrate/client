...
div(class="flex items-center gap-2 px-3 py-2.5 cursor-grab active:cursor-grabbing")
  ...
  div(class="flex-1 min-w-0")
    ...
    p(v-if="thresholdCount" class="text-xs text-gray-400 truncate")
      span(class="font-numeric tabular-nums") {{ thresholdCount }}
      |
      | threshold{{ thresholdCount !== 1 ? 's' : '' }}
  slot(name="actions")
</template>
...
@Component
export default class FeatureChipCostAwareRouting extends Vue {
  @StringProp(true)
  ...
  public get thresholdCount(): number {
    const t = routerFeatureConfig<FT.FeatureCostAwareRouting>(this.feature).thresholds
    return t?.length ?? 0
  }
}
</script>
