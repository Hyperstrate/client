...
@Component
export default class FeatureChipResponseFingerprinting extends Vue {
  @StringProp(true)
  ...
  public get subtitle(): string | undefined {
    const cfg = routerFeatureConfig<FT.FeatureResponseFingerprinting>(this.feature)
    const window = cfg.window_size
    const sigma = cfg.alert_threshold
    if (window !== undefined) return `${window}-sample window · ${sigma ?? 3}σ alert`
    return undefined
  }
}
</script>
