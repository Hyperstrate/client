...
div(class="flex items-center gap-2 px-3 py-2.5 cursor-grab active:cursor-grabbing")
  ...
  div(class="flex-1 min-w-0")
    ...
    p(v-if="promptCount" class="text-xs text-gray-400 truncate")
      span(class="font-numeric tabular-nums") {{ promptCount }}
      |
      | follow-up{{ promptCount !== 1 ? 's' : '' }}
  slot(name="actions")
</template>
...
@Component
export default class FeatureChipResponsePrefetch extends Vue {
  @StringProp(true)
  ...
  public get promptCount(): number {
    const p = routerFeatureConfig<FT.FeatureResponsePrefetch>(this.feature).follow_up_prompts
    return p?.length ?? 0
  }
}
</script>
