...
div(class="flex items-center gap-2 px-3 py-2.5 cursor-grab active:cursor-grabbing")
  ...
  div(class="flex-1 min-w-0")
    ...
    p(v-if="maxRetries" class="text-xs text-gray-400 truncate")
      | up to
      span(class="font-numeric tabular-nums") {{ maxRetries }}
      |
      | retries
  slot(name="actions")
</template>
...
@Component
export default class FeatureChipRetry extends Vue {
  @StringProp(true)
  ...
  public get maxRetries(): number | undefined {
    const v = routerFeatureConfig<FT.FeatureRetry>(this.feature).max_retries
